...
- NOT razor-sharp CGI, NOT photorealistic bread textures
...
Village bakery owned by an anonymous warm-brown round Mr. Men baker. One half of the famous "빵집 vs 꽃집 sidewalk dispute" in Ch1. Should feel inviting AND visually anchored next to its rival, the flower shop (the spatial relationship matters — they fight over the sidewalk patch between them).
...
3. **Large rectangular front window** — INSIDE the window: simple oval brown bread-loaf shapes on a wooden shelf (3-5 oval shapes — round country loaf, baguette oval, small bun ovals — flat brown shades with PIXAR-style warm soft glow suggesting fresh bread). One large round bread loaf in center as focal piece. Window glass catches subtle reflection.

4. **Front door** — warm-brown wooden rectangle with small round window in upper portion, subtle paint and grain texture.

5. **Awning** — striped RED and WHITE (vivid, distinctive from cafe's cream/brown), soft cloth texture with hanging folds.

6. **Hanging wooden sign** above door — BLANK rectangle OR small bread-loaf icon (NO text), iron bracket holding it.

7. **Small chalkboard easel** on sidewalk — slate-gray on wooden stand, abstract chalk scribbles.

8. **Cobblestone sidewalk** with dimensional depth.

9. **Hint of flower shop on RIGHT edge** — small partial view of pink wall + one green flower bucket peeking in from right edge. Establishes "they are next door" — critical for dispute scene continuity.
...
- 건물 색: 따뜻한 골든 베이지 (카페와 시각 구분)
- 시그니처: **창문 안 빵 디스플레이** + **빨강/흰색 어닝** (꽃집 초록/흰색과 대비)
- 공간 단서: **우측에 꽃집 살짝** (분쟁 구조 미리 시각화)
- PIXAR 재질: 천 어닝 폴드 + 빵 따뜻한 글로우 + 유리창 반사 + 돌 기초
...
[05_flower_shop.png](05_flower_shop.md) 와 항상 함께
